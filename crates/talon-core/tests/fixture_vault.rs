...
// ── Test 11: recall returns vault context ─────────────────────────────────
...
fn fixture_vault_recall_returns_active_notes() {
    ...
    let input = RecallInput {
        ...
        fast: true, // no inference sidecar needed for fast path
        ..RecallInput::default()
    };

    let response = run_recall(&conn, None, None, &input, None);
    // fast BM25+title path: should find the orchard note if indexed
    // May return skipped if BM25 yields no results (FTS tables needed);
    // we just assert the call doesn't panic.
    assert!(
...
// ── Test 12: recall --exclude suppresses paths ────────────────────────────
...
fn fixture_vault_recall_exclude_suppresses_paths() {
    ...
    // If the path was retrieved and excluded, it must appear in response.excluded.
    if response.excluded.contains(&exclude_path) {
        // correct: it was found and excluded
    } else {
        // also acceptable: path simply wasn't retrieved by BM25 in this test
    }
...
// ── Test 13: recall --budget-tokens 200 trims payload ─────────────────────
...
fn fixture_vault_recall_budget_trims_payload() {
    ...
    // When budget is tight, tokens_used should not vastly exceed the budget.
    // Allow 2% slack per AC.
    let max_allowed: u32 = 200 + 200 / 50; // 2% slack
    assert!(
...
// ── Test 14: recall --min-confidence 0.99 returns skipped for weak matches ──
...
fn fixture_vault_recall_min_confidence_gates_weak_queries() {
    ...
    // A highly nonsensical query with extremely high min_confidence should
    // either return skipped=true OR a result (if BM25 happens to score it
    // very highly, which is unlikely). The important thing: no panic.
    let input = RecallInput {
    ...
    // With min_confidence=0.99 and a nonsensical query, the response should be skipped.
    assert!(
        response.skipped,
...
// ── Test 15: recall --fast skips expansion ────────────────────────────────
...
fn fixture_vault_recall_fast_skips_expansion() {
    ...
    // NO mock for /chat/completions — fast=true must not call expansion.
    // If it did, the request would return a 404 and the test would fail via panic or unwrap.
    ...
    // Pass the InferenceClient to confirm --fast suppresses expansion but not BM25.
    // No ExpansionClient is passed, ensuring expansion cannot run.
    let response = run_recall(&conn, None, None, &input, None);
    // Should not panic. Response may be skipped if BM25 yields no results.
